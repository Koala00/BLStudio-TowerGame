...
public struct HexCoord
{
    ...
    public static HexCoord FromOffsetEvenR(int row, int col)
    {
        return new HexCoord(col - (row + (row & 1)) / 2, row);
    }

    public static HexCoord FromOffsetOddR(int row, int col)
    {
        return new HexCoord(col - (row - (row & 1)) / 2, row);
    }
    
    public Vector2 EvenRToOffset()
    {
        int col = q + (r + (r & 1)) / 2;
        int row = q;
    ...
    /// <summary>
    /// Get the Unity position (2d) of a corner vertex.
    /// </summary>
    /// <remarks>
    ...
    {
        return CornerVector(index) + Position2d();
    }

    /// <summary>
    /// Get the Unity position (3d) of a corner vertex.
    /// </summary>
    /// <remarks>
    /// Corner 0 is at the upper right, others proceed counterclockwise.
    /// </remarks>
    /// <param name="index">Index of the desired corner. Cyclically constrained 0..5.</param>
    public Vector3 Corner3d(int index)
    {
        var v = CornerVector(index);
        return new Vector3(v.x, 0, v.y) + Position3d();
    }
...
(q <= -2 * r) ? 3 : 2;
    ...
    public static HexCoord AtPosition3d(Vector3 position)
    { return AtPosition(new Vector2(position.x, position.z)); }
    ...
    public static HexCoord[] CartesianRectangleBounds(Vector2 cornerA, Vector2 cornerB)
    {
        ...
        Vector2 pos = results[0].Position2d();
        if (pos.y - 0.5f >= min.y)
            results[0] += neighbors[4];
        else if (pos.x >= min.x)
            results[0] += neighbors[3];
        pos = results[1].Position2d();
        if (pos.y + 0.5f <= max.y)
